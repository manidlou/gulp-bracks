...
describe("parse bracks-style html with nested elements", function() {
  ...
  it("should be equal to correct regular html with nested elements", function(done) {
    bracks_parser_tester(path.join(__dirname, 'bracks'), function(err, ok) {
      if (err !== null) {
        throw err;
      } else if (ok === 0) {
        result_html = fs.readFileSync(path.join(__dirname, 'views', 'html-doc.html'), 'utf8');
        correct_html = fs.readFileSync(path.join(__dirname, 'preparsed', 'correct-html.html'), 'utf8');
  ...
  it("should be equal to correct regular ejs", function(done) {
    bracks_parser_tester(path.join(__dirname, 'bracks'), function(err, ok) {
      if (err !== null) {
        ...
        result_ejs = fs.readFileSync(path.join(__dirname, 'views', 'ejs-doc.ejs'), 'utf8');
        correct_ejs = fs.readFileSync(path.join(__dirname, 'preparsed', 'correct-ejs.ejs'), 'utf8');
